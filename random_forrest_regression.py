...
train, test = sklearn.model_selection.train_test_split(dataset, train_size=0.8, random_state=42)  # Set random_state
...
for output_parameter in ["BTE_percentage", "BSFC_kg_per_kWh", "NOx_ppm", "HC_ppm", "CO_percentage", "CO2_percentage"]:
    ...
    model = sklearn.ensemble.RandomForestRegressor()  # Set random_state
    ...
    plt.text(min_y, max_y * 0.95, f"MAE Train: {sklearn.metrics.mean_absolute_error(train_pred, y_train[output_parameter]):.3f} MAE Test: {sklearn.metrics.mean_absolute_error(test_pred, y_test[output_parameter]):.3f}")
    plt.text(min_y, max_y * 0.9, f"R2 Train: {sklearn.metrics.r2_score(train_pred, y_train[output_parameter]):.3f} R2 Test: {sklearn.metrics.r2_score(test_pred, y_test[output_parameter]):.3f}")
    plt.plot(
